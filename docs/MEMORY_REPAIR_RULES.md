...
Example: a ChromaDB matcher that was meant to flag 14 rows might pick up
7 more if the query changes or a threshold shifts. The new count
...
A backup is not verified until it can be opened and all collections
read back with their counts matching the live database. Opening the
copy, querying it, and confirming the row counts is the standard.
...
## 12. No hardcoded database paths

A script must never hardcode a database path. Take it as a required
argument or config parameter, and fail loudly if the path does not
exist. `chromadb.PersistentClient(path=...)` creates a new empty
database at a typo'd path instead of raising an error, so a wrong
path is silent data loss.

## 13. Memory health report

...
from backend.core.tools.registry import REGISTRY, _resolve_name
...
def verify(user_query: str, call: dict) -> VerificationResult:
    """Verify a proposed tool call for hallucinations, schema validity, and relevance."""
    ...
    if not resolved:
        return VerificationResult(False, error=f"Tool {name!r} not found in registry (hallucination).")

    tool_obj = REGISTRY[resolved]

    # 2. Malformed Check (Schema)
    ...
    # 3. Confidence / Relevance Check (Secondary Model)
    if not _secondary_check(user_query, resolved, args, reasoning):
        return VerificationResult(False, error="Action rejected by secondary verification: logic or relevance mismatch.")
